...
def main():
    ...
    if args.sparsity_ratio != 0:
        ...
        if args.prune_method == "wanda":
            # Use the module names specified in the command line
            if args.module_names:
                module_names = args.module_names
            else:
                # Default modules to prune if none specified
                module_names = ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj", 
                                "self_attn.dense", "mlp.fc1", "mlp.fc2"]

            for module_name in module_names:
                print(f"Pruning module {module_name}")
                prune_wanda(args, model, tokenizer, device, module_names=[module_name], prune_n=prune_n, prune_m=prune_m)
